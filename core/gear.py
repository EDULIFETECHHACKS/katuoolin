...
def load_category(key):
	...
	if int(key) in categories.keys():
		...
		while action == False:
			try:
				option = raw_input(": katuoolin (%s%s%s) > " %(yellow, site, reset))
...
def add_repository():
	...
	"""
	if os.path.exists("/etc/apt/sources.list.d/katuoolin.list"):
		add_key()
	else:
		try:
			f = open("/etc/apt/sources.list.d/katuoolin.list", "wb")
...
def delete_repository():
	...
	"""
	repository = "/etc/apt/sources.list.d/katuoolin.list"
...
def add_key():
	...
	if os.path.exists(tmp_key):
		...
		f.close()
		print green + "\n[+] Add keyserver\n" + reset
		os.system('apt-get update -o Dir::Etc::sourcelist="sources.list.d/katuoolin.list" -o Dir::Etc::sourceparts="-" -o apt::Get::List-Cleanup="0"')
		print green + "\n[+] Update\n" + reset
...
$$ |$$  /  $$$$$$\  $$$$$$\  |$$   $$|  $$$$$$\   $$$$$$\  $$ |$$\ $$$$$$$\  
$$$$$  /   \____$$\ \_$$  _| |$$   $$| $$  __$$\ $$  __$$\ $$ |$$ |$$  __$$\ 
$$  $$<    $$$$$$$ |\033[1;36m Kali linux tools installer all in 1  |$$ |$$ |$$ |  $$ |
$$ |\$$\  $$  __$$ |  $$ |$$\|$$   $$| $$ |  $$ |$$ |  $$ |$$ |$$ |$$ |  $$ |
$$ | \$$\ \$$$$$$$ |  \$$$$  | $$$$$ ||\$$$$$$  |\$$$$$$  |$$ |$$ |$$ |  $$ |
\__|  \__| \_______|   \____/|\_____/|  \______/  \______/ \__|\__|\__|  \__| V2.0\033[1;m


 %s+ -- -- +=[ Original project: https://github.com/edulifetechhacks/katuoolin | EDULIFETECHHACKS
 + -- -- +=[ %s Tools%s""" %(cyan,reset,cyan,version,reset,green,tools,reset)
 	print options

def update():
	"""
	Update 'katuoolin' with: git pull
	"""
 	try:
		os.system('git pull')
		print  yellow + "W: Please restart katuoolin" + reset
	except:
...
: 2, update         Update katuoolin (git pull)
: 3, help           Show help
: 4, exit           Exit katuoolin"""
	else:
